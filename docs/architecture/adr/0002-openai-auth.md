...
Returning either credential to UI state would enlarge the trusted surface and
make accidental disclosure through view logging more likely.

## Decision

OpenAI is a privileged first-party provider implemented as a typed Rust service
behind `cellar-ai`.

API-key mode loads `ai:openai` from `cellar-secrets` in Rust and calls the
OpenAI Responses API directly. The key is never returned to GPUI view state.
...
item that still starts. Only the final assistant text and token counts reach
GPUI view state.
...
- OpenAI credentials never enter GPUI view state.
...
- Other providers use the same native credential boundary.

## Follow-up

- Bundle, sign, and version-pin the Codex app-server sidecar for release builds.
- Stream response deltas over bounded native channels and expose turn cancellation.
