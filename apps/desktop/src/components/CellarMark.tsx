...
type CellarMarkProps = {
  className?: string;
  accented?: boolean;
};

export function CellarMark({ className, accented = false }: CellarMarkProps) {
  const gradientId = useId().replace(/:/g, "");
  const paint = accented ? `url(#${gradientId})` : "currentColor";
        ...
        <defs>
          <linearGradient id={gradientId} x1="0" y1="0" x2="1" y2="1">
            <stop
              ...
              style={{
                stopColor:
                  "color-mix(in oklab, var(--accent) 58%, white)",
              }}
            />
